...
if __name__ == "__main__":
    ...
    def traverse_printer(nodes: List[Node]):
        return "\n".join(list(map(lambda node: str(node), nodes)))
    ...
    print("Дерево А в прямом порядке:\n" + traverse_printer(tree_a.traverse_preorder()))
    print(
        "Дерево B в симметричном порядке:\n"
        + traverse_printer(tree_b.traverse_inorder())
    )
    ...
    print(
        "Дерево А после удаления узлов не содержащихся в дереве В:\n"
        + traverse_printer(tree_a.traverse_preorder())
    )
