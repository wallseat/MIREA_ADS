...
if __name__ == "__main__":
    ...
    for _ in range(20):
        value = randint(-99, 99)
        tree_a.insert(value)
        if not randint(1, 29) % 4:
            tree_b.insert(value)

    print(
        f"Дерево А в обратном порядке: {tree_a.traverse_postorder()}\nВысота: {tree_a.height()}"
    )
    print(
        f"Дерево B в симметричном порядке: {tree_b.traverse_inorder()}\nВысота: {tree_b.height()}"
    )
    ...
    print(
        f"Дерево С созданное из дерева А исключая элементы дерева B: {tree_c.traverse_inorder()}\nВысота: {tree_c.height()}"
    )
