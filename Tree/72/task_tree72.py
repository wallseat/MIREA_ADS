...
if __name__ == "__main__":
    ...
    print("Дерево A в прямом порядке:", tree_a.traverse_preorder())
    print("Дерево B в симметричном порядке:", tree_b.traverse_inorder())

    for v in tree_b.traverse_postorder():
        tree_a.insert(v.value)

    print(
        f"Дерево A в прямом порядке  после добавления элементов дерева B в обратном порядке:\n"
