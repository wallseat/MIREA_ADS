...
if __name__ == "__main__":
    ...
    print(
        f"Дерево А в прямом порядке: {tree_a.traverse_preorder()}, Высота {tree_a.height()}"
    )
    print(
        f"Дерево B в симметричном порядке: {tree_b.traverse_inorder()}, Высота {tree_b.height()}"
    )

    for node in chain(tree_a.traverse_inorder(), tree_b.traverse_postorder()):
        tree_c.insert(node.value)

    print(
        f"Дерево C после добавления элементов дерева B в дерево A в обратном порядке:\n{tree_c.traverse_inorder()}, Высота {tree_c.height()}"
    )
