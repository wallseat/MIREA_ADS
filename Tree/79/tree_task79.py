...
if __name__ == "__main__":
    ...
    for _ in range(15):
        v = random.randint(-99, 100)
        tree_a.insert(v)
        
        if random.randint(0, 99) % 10 >= 5:
            tree_b.insert(v)
    ...
    a_elems = tree_a.traverse_inorder()
    
    for elem in a_elems:
        if tree_b.find(elem.value) is None:
            tree_a.remove(elem.value)
            
    print("Дерево А без элементов отстутствующих в дереве B: " + str(tree_b.traverse_preorder()))
