...
def find_graph_k(matrix: List[List[int]]) -> int:
    graph = []
    dim = len(matrix)

    def bfs(s: int, t: int, parent: List[int]) -> bool:
        visited = [False] * dim
...
if __name__ == "__main__":
    graph = Graph()
    graph.load("graph_task80.json")

    k = find_graph_k(graph.to_adj_matrix())
    print(f"K-связность графа равна: {k}")
