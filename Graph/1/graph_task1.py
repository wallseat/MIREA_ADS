...
class Graph:
    ...
    def add_vertex_with_index(self,v_name,v_index):
        ...
        if (not v_name in self.vertex_names) and (not v_index in self.vertex_names.values()):
            ...
            print(f"{TextColors.FAIL}///ERROR///{TextColors.ENDC} ВЕРШИНА С НАЗВАНИЕМ '{v_name}' УЖЕ ИСПОЛЬЗУЕТСЯ!")
        else:
    ...
    def add_edge(self,v1_name,v2_name):
        if v1_name and v2_name in self.vertex_names:
            ...
            print(f"{TextColors.FAIL}///ERROR///{TextColors.ENDC} НЕТ ВЕРШИН С НАЗВАНИЯМИ '{v1_name}' и '{v2_name}'!")
    ...
    def last_in_buff(self,buff):
        ...
        except: last_el_buff.append(buff[0][1])
    ...
    def cleared_cycles(self,cycle_list,cycle,original_lists):
        ...
        cycle+=list(self.vertex_names.keys())[list(self.vertex_names.values()).index(cycle_list[len(cycle_list)-1][1])]
