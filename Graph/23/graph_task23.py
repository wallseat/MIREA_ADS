...
def get_diameter_and_chains(adj_matrix: List[List[int]]) -> List[List[int]]:
    n = len(adj_matrix)

    def _bfs():
        """
        Find matrix of shortest paths between all pairs of vertices
        """
...
if __name__ == "__main__":
    graph = Graph()
    graph.load("graph_task23.json")

    diameter, chains = get_diameter_and_chains(graph.to_adj_matrix())
