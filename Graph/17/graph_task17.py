...
class Graph:
    ...
    def _load_adj_matrix(self, data: List[List[int]], dim: int) -> None:
        ...
        for i, adj in enumerate(data):
            for j, edge_len in enumerate(adj):
                if edge_len > 0:
...
def find_strongly_connected_components(adj_matrix: List[List[int]]) -> List[List[int]]:
...
if __name__ == "__main__":
    matrix_file = "graph_task17.json"

    graph = Graph()
    graph.load(matrix_file)

    adj_matrix = graph.to_adj_matrix()
    print("Матрица смежности:\n" + "\n".join([" ".join([str(j) for j in i]) for i in adj_matrix]))
    comps = find_strongly_connected_components(graph.to_adj_matrix())
    print("Компоненты сильной связности:\n" + "\n".join([str(i) for i in comps]))
