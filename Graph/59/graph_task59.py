...
def find_simple_paths(
    adj_matrix: List[List[int]], start: int, end: int, path: List[int] = []
) -> List[List[int]]:
    path = path + [start]
    if start == end:
        return [path]
    if not any(adj_matrix[start]):
        return []
    paths = []
    for i, is_adjacent in enumerate(adj_matrix[start]):
        if is_adjacent and i not in path:
            new_paths = find_simple_paths(adj_matrix, i, end, path)
...
if __name__ == "__main__":
    ...
    graph = Graph()
    graph.load("graph_task59.json")

    if len(sys.argv) < 3:
        print("Необходимо указать две вершины!")
        exit(1)

    v1, v2 = sys.argv[1], sys.argv[2]

    paths = find_simple_paths(graph.to_adj_matrix(), int(v1), int(v2))

    print(f"Все простые пути из вершины {v1} в вершину {v2}:")
