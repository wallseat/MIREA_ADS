...
def find_center(distance_matrix: List[List[int]]) -> int:
    max_distances = []
...
def find_chains(matrix: List[List[int]], v: int, distance: int) -> List[List[int]]:
    stack = []
    chains = []
...
if __name__ == "__main__":
    ...
    graph = Graph()
    graph.load("graph_task27.json")

    distance_matrix = BFS(graph.to_adj_matrix())
    ...
    chains = find_chains(graph.to_adj_matrix(), center_v, radius)
