...
def swap(stack: Stack, pos1: int, pos2: int): # 8n^2 + 14n - 12
    ...
    push_by_pos(stack, pop_by_pos(stack, pos2 - 1), pos1) # 2n^2 + 4n + 2 + 2n^2 + 3n - 8
...
def slice_stack(stack: Stack[VT], l: int = 0, r: int = -1) -> Stack[VT]: # 4 + 4 + n^2 + 3n - 5 + n + n^2 + 2n = 2n^2 + 6n + 3
    stack_size = stack.size # 2
...
def merge_sort(stack: Stack): # 4(n^2)log(n) + 8n + 2log(nlog(n)) + 13
    def _merge(left: Stack, right: Stack) -> Stack: # 1 + 2 + k^3 + m^3 + k^3 + 2k + m^3 + 2m = 2k^3 + 2m^3 + 2k + 2m + 3
        result = Stack() # 1
        i, j = 0, 0 # 2
