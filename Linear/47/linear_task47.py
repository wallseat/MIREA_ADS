...
def merge_sort(queue: Queue[VT]) -> Queue[VT]:  # 2n^2 + 2nlog(n) + 14n + 11
    if queue.size <= 1:  # 2
        return queue

    left = slice_(queue, 0, queue.size // 2 - 1)  # 3n + 2
    right = slice_(queue, queue.size // 2)  # 4n + 2

    left = merge_sort(left)  # ~= nlog(n)
    right = merge_sort(right)  # ~= nlog(n)

    result = Queue[VT]()  # 1

    while not left.is_empty() and not right.is_empty():  # n * (
        if seek(left, 0) <= seek(right, 0):  # 2n + 2
            result.push(left.pop())  # 2
        else:
            result.push(right.pop())  # 2
    # ) = 2n^2 + 4n

    while not left.is_empty():  # m (where m << n) * (
        result.push(left.pop())  # 2
    # ) ~= 2

    while not right.is_empty():  # k (where k << n) * (
        result.push(right.pop())  # 2
    # ) ~= 2

    for _ in range(queue.size):  # n * (
        queue.pop()  # 1
        queue.push(result.pop())  # 2
    # ) = 3n

    queue._n_op += left.n_op + right.n_op
