...
def shell_sort(stack: Stack[VT]) -> Stack[VT]:  # 138n^2 * log(n) - 90n * log(n) + 6
    last_index = stack.size - 1  # 3
    step = stack.size // 2  # 3
    while step > 0:  # log(n) * (
        for i in range(step, last_index + 1, 1):  # n * (
            j = i  # 1
            delta = j - step  # 2
            while delta >= 0 and seek(stack, delta) < seek(
                stack, j
            ):  # k (where k << n) * (
                swap(stack, delta, j)  # 138n - 96
                j = delta  # 1
                delta = j - step  # 2
            # ) = 138n - 93
        # ) = 138n^2 - 90n
        step //= 2
    # ) = 138n^2 * log(n) - 90n * log(n) + 6
