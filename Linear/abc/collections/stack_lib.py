...
def push_front(stack: Stack[VT], el: VT) -> None:  # 2
    stack.push(el)  # 2


def push_back(stack: Stack[VT], el: VT) -> None:  # 14n + 4
    buffer = Stack[VT]()  # 2
...
def pop_front(stack: Stack[VT]) -> VT:  # 5
    return stack.pop()  # 5


def pop_back(stack: Stack[VT]) -> VT:  # 14n + 7
    buffer = Stack[VT]()  # 2
