...
def seek(stack: Stack[VT], pos: int) -> VT:  # $CXDEF: 44n + 11$
...
def push_by_pos(stack: Stack[VT], el: VT, pos: int) -> None:  # $CXDEF: 44n + 20$
    assert pos <= stack.size and pos >= 0, "Invalid position!"  # 5
...
def pop_by_pos(stack: Stack[VT], pos: int) -> VT:  # $CXDEF: 44n + 16$
    assert pos <= stack.size and pos >= 0, "Invalid position!"  # 5
...
def push_back(stack: Stack[VT], el: VT) -> None:  # $CXDEF: 46n + 13$
    buffer = Stack[VT]()  # 2
...
def pop_back(stack: Stack[VT]) -> VT:  # $CXDEF: 44n + 11$
    buffer = Stack[VT]()  # 2
...
def swap(stack: Stack, pos1: int, pos2: int) -> None:  # $CXDEF: 176n + 72$
    temp = pop_by_pos(stack, pos1)  #  44n + 16
    push_by_pos(stack, pop_by_pos(stack, pos2 - 1), pos1)  # 88n + 36
    push_by_pos(stack, temp, pos2)  # 44n + 20


def slice_(stack: Stack[VT], l: int = 0, r: int = -1) -> Stack[VT]:  # $CXDEF: 51n + 69$
