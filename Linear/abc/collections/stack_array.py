...
def seek(stack: Stack[VT], pos: int) -> VT:  # 44n + 11
...
def push_by_pos(stack: Stack[VT], el: VT, pos: int) -> None:  # 44n + 20
    assert pos <= stack.size and pos >= 0, "Invalid position!"  # 5
...
def pop_by_pos(stack: Stack[VT], pos: int) -> VT:  # 44n + 16
    assert pos <= stack.size and pos >= 0, "Invalid position!"  # 5
