...
def seek(queue: Queue[VT], pos: int) -> VT: # 2n^2 + 10n + 2
    ...
    for _ in range(queue.size - pos): # (2n + 10) * (n - pos) =  2n^2 + 10n - 2n * pos - 10pos
        rotate(queue)
...
def push_by_pos(queue: Queue[VT], el: VT, pos: int): # 2n^2 + 8n - 4 
    ...
    for _ in range(queue.size - pos - 1): # (2n + 10) * (n - pos - 1) =  2n^2 + 10n - 2n * pos - 10pos - 2n - 10
        rotate(queue)


def swap(queue: Queue, pos1: int, pos2: int): # 8n^2 + 36n + 4
    temp = pop_by_pos(queue, pos1) # 2n^2 + 10n + 6
    push_by_pos(queue, pop_by_pos(queue, pos2 - 1), pos1) # 2n^2 + 8n - 4 + 2n^2 + 10n + 6
...
def heapify(queue: Queue, n: int, i: int): 
    ...
    if l < n and seek(queue, i) < seek(queue, l): # 1 + 1 + 2n^2 + 10n + 1 + 2n^2 + 10n + 2 = 4n^2 + 20n + 5
        largest = l # 1
  
    if r < n and seek(queue, largest) < seek(queue, r): # 1 + 1 + 2n^2 + 10n + 1 + 2n^2 + 10n = 4n^2 + 20n + 5
        largest = r # 1
...
def heap_sort(queue: Queue): 
    ...
    for i in range(n // 2 - 1, -1, -1): # (n / 2) * (16n^2 + 76n + 24) * log(n * log(n)) ~= 8n^3 * log(n * log(n)) + 38n^2 + 12n
        heapify(queue, n, i) 
