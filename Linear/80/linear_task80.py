...
def bubble_sort(queue: Stack[VT]) -> Stack[VT]:
    for i in range(queue.size):  # n * (
        for j in range(i, queue.size):  # n * (
            if seek(queue, i) < seek(queue, j):  # 56n - 44
                swap(queue, i, j)  # 112n - 12
        # ) = 168n^2 - 56n + 1
    # ) = 168n^3 - 56n^2 + n + 1
