...
class DoubleLinkedList:
    ...
    def __init__(self, head=None, tail=None, length=0):
        ...
        #ссылка на голову
        self.head = head;  self.n_op+=1
        #ссылка на хвост
        self.tail = tail;  self.n_op+=1
        #длина дека
        self.length = length;  self.n_op+=1
    ...
    def pop_right(self):
        ...
        #если элемент в деке только 1
            ...
            self.tail = None;  self.n_op+=1
            #ссылки на голову больше нет
            self.head = None;  self.n_op+=1
...
def max(dll):
    ...
    max_elem = -1;  dll.n_op+=1
    #инициализация буфера для отгрузки элементов из дека
    buff = DoubleLinkedList(); dll.n_op+=4
    ...
    while dll.length!=0:
        dll.n_op+=1
        #получение самого последнего элемента дека
        right_elem = dll.right_elem();  dll.n_op+=2

        #добавление этого элемента слева в буфер
        buff.push_left(right_elem);  dll.n_op+=2
        #удаление последнего узла из дека
        dll.pop_right();  dll.n_op+=1

        #если последний элемент дека больше максимального
        if right_elem> max_elem:
            dll.n_op+=1
            #максимальный элемент становится новым найденным
            max_elem = right_elem;  dll.n_op+=1
    ...
    while buff.length!=0:
        ...
        right_elem = buff.right_elem();  dll.n_op+=2
        #добавление этого элемента слева обратно в дек
        dll.push_left(right_elem);  dll.n_op+=2
        #удаление последнего узла из буфера
        buff.pop_right();  dll.n_op+=1

    #возврат максимального элемента
    return max_elem;  dll.n_op+=1
...
def counting_sort(dll):
    ...
    counting_buff = [0 for i in range(max_elem+1)];  dll.n_op+=1
    ...
    while dll.length!=0:
        ...
        counting_buff[right_elem]+=1;  dll.n_op+=3
        #добавление этого элемента слева в буфер
        buff.push_left(right_elem);  dll.n_op+=2
        #удаление последнего узла из дека
        dll.pop_right();  dll.n_op+=1
    ...
    for i in range(max_elem+1):
            ...
            for j in range(counting_buff[i]): #сколько раз потвторяется
                ...
                dll.pop_left();  dll.n_op+=1
                #добавление нового узла в дек справа
                dll.push_right(i);  dll.n_op+=2
...
for i in range(10):
    ...
    max_elem = max(dll); dll.n_op+=3

    #сортировка дека
    counting_sort(dll); dll.n_op+=2
    # dll.print_list()
    ...
    print(f"{i}) Count of elements: {dll.length};   Total time: {this_time} sec;   Num of operations: {dll.num_of_operations()};")
