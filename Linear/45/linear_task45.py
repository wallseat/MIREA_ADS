...
def slice_queue(queue: Queue[VT], l: int = 0, r: int = -1) -> Queue[VT]: # 1 + 1 + 3 + 2n + 4 + 2n + 6 + 2n^2 - 4 = 2n^2 + 4n + 15
    q = Queue[VT]() # 1
...
def merge_sort(queue: Queue): # 4(n^2)log(n) + 8n + 2log(nlog(n)) + 13
    def _merge(left: Queue, right: Queue) -> Queue: # 1 + 2 + k^3 + m^3 + k^3 + 2k + m^3 + 2m = 2k^3 + 2m^3 + 2k + 2m + 3
        result = Queue() # 1
        i, j = 0, 0 # 2
